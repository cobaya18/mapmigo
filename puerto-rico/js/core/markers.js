  ...
  state.places.forEach((place, index) => {
    ...
    if (!isMobile()) {
      ...
      marker.on("popupopen", (e) => {
        ...
        if (!popupEl) return;
        const seeBtn = popupEl.querySelector(".popup-see-more");
        const section = popupEl.querySelector(".popup-more-section");
        if (seeBtn && section) {
          seeBtn.addEventListener("click", () => {
            section.classList.toggle("expanded");
            seeBtn.textContent = section.classList.contains("expanded") ? "See less" : "See more";
          });
        }
        ...
        if (copyBtn) {
          copyBtn.addEventListener("click", async () => {
            try { await navigator.clipboard.writeText(e.popup._source?.options?.alt || ""); } catch(e){}
          });
        }
        ...
        if (nativeBtn) {
          nativeBtn.addEventListener("click", async ()=> {
            if (navigator.share) {
              navigator.share({ title: place.title, url: e.popup._source?.options?.alt||"" });
            }
          });
        }
      });
...
function buildPopupHtml(place, key, url) {
  ...
  if (place.image) {
    html += `
      <div class="popup-image-wrapper skeleton">
        <img src="${place.image}" class="popup-image" 
  ...
  html += `
    <button class="popup-see-more">See more</button>
    <div class="popup-more-section collapsed">
      <p class="popup-more-text">${place.longDescription || ""}</p>
    </div>
  `;
