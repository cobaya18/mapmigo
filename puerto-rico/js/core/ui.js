...
function initSidebarToggle() {
  ...
  function toggleSidebar() {
    // On desktop we allow collapsing the sidebar.
    // On mobile we rely on the dedicated filter bottom sheet instead.
    if (isMobile()) return;
...
function initFilterSheet() {
  ...
  const openHandle = document.getElementById("mobileFilterHandle");

  if (!filterSheet || !openHandle) return;

  const sheetHandle = filterSheet.querySelector(".sheet-handle");
  ...
  function closeSheet() {
    ...
    filterSheet.classList.remove("expanded");
    filterSheet.style.transform = "";
  }

  openHandle.addEventListener("click", () => {
    if (!isMobile()) return;
    openSheet();
  });
  ...
  // Drag-to-close on mobile (similar to place sheet)
  let dragStartY = null;
  let currentY = null;
  let dragging = false;
  ...
  if (sheetHandle) {
    sheetHandle.addEventListener("touchstart", sheetTouchStart, { passive: true });
    sheetHandle.addEventListener("touchmove", sheetTouchMove, { passive: true });
    sheetHandle.addEventListener("touchend", sheetTouchEnd);
    sheetHandle.addEventListener("touchcancel", sheetTouchEnd);
  }
}
