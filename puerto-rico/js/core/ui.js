...
function initPlaceSheet() {
  ...
  const sheetImageCredit = document.getElementById("sheetImageCredit");
  ...
  const creditText = getImageCreditText(place);
  if (sheetImageCredit) {
    if (creditText && (place.image_url || place.image)) {
      sheetImageCredit.textContent = creditText;
      sheetImageCredit.style.display = "block";
    } else {
      sheetImageCredit.textContent = "";
      sheetImageCredit.style.display = "none";
    }
  }
