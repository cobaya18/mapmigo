...
document.addEventListener("DOMContentLoaded",()=>{
  const trig=document.getElementById("filterSheetTrigger");
  if(trig){
    trig.addEventListener("click",()=>{
      window.dispatchEvent(new Event("openFilterSheet"));
    });
  }
});


// DRAG TO EXPAND FILTER SHEET (50%)
(function(){
  const fs = document.getElementById("filterSheet");
  if(!fs) return;
  let startY = 0;
  let currentY = 0;
  let dragging = false;
  const openHeight = window.innerHeight * 0.5;
...
// DRAG + TAP FILTER SHEET
(function(){
  const fs=document.getElementById("filterSheet");
  const handle=fs.querySelector(".filter-sheet-handle");
  if(!fs||!handle) return;

  handle.addEventListener("click",()=>fs.classList.add("open"));
  ...
  handle.addEventListener("touchmove",e=>{
    ...
    currentY=e.touches[0].clientY;
    const diff=startY-currentY;
    if(diff>20) fs.classList.add("open");
    if(diff<-20) fs.classList.remove("open");
  });
