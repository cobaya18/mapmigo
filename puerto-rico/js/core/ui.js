...
function initSidebarToggle() {
  ...
  const bottomHandle = document.getElementById("mobileFilterHandle");
  ...
  function updateHandleVisibility() {
    ...
    // On mobile: show handle only when sidebar is closed
  ...
  // Bottom handle on mobile to open the filter sheet from the bottom edge
  if (bottomHandle) {
  ...
  // Drag-to-close behavior on mobile using the internal sidebar handle
  ...
  if (sidebarHandle) {
    ...
    sidebarHandle.addEventListener("touchstart", handleTouchStart, { passive: true });
    sidebarHandle.addEventListener("touchmove", handleTouchMove, { passive: true });
    sidebarHandle.addEventListener("touchend", handleTouchEnd);
    sidebarHandle.addEventListener("touchcancel", handleTouchEnd);
  }
