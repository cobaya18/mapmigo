...
function initFilterSheet() {
  ...
  const summaryBtn = document.getElementById("filterSheetSummary");

  if (!filterSheet || !summaryBtn) return;

  const isMobile = () => window.innerWidth <= 768;

  function toggleSheet() {
    if (!isMobile()) return;
    filterSheet.classList.toggle("filter-sheet--expanded");
  }

  summaryBtn.addEventListener("click", toggleSheet);

  const handle = filterSheet.querySelector(".sheet-handle");
  if (handle) {
  ...
  window.addEventListener("resize", () => {
    if (!isMobile()) {
      filterSheet.classList.remove("filter-sheet--expanded");
    }
  });
}
