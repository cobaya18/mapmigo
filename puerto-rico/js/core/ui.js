...
function applyTheme(theme) {
  ...
  try {
    localStorage.setItem("mapmigo_theme", theme);
  } catch (e) {
...
function detectInitialTheme() {
  try {
    const stored = localStorage.getItem("mapmigo_theme");
...
function initTopBarDelegation() {
  document.addEventListener("click", (event) => {
    const target = event.target;
    if (!target) return;

    const savedBtn = target.closest("#openSavedView");
    if (savedBtn && window.__mmOpenSavedView) {
      event.preventDefault();
      window.__mmOpenSavedView();
      return;
    }

    const listBtn = target.closest("#openListView");
    if (listBtn && window.__mmOpenListView) {
      event.preventDefault();
      window.__mmOpenListView();
      return;
    }
  });
}
...
function initListView() {
  ...
  window.__mmOpenListView = openListView;
  window.__mmOpenSavedView = openSavedView;
  ...
  if (openListViewBtn) openListViewBtn.addEventListener("click", (e) => { e.preventDefault(); openListView(); });
  if (closeListViewBtn) closeListViewBtn.addEventListener("click", (e) => { e.preventDefault(); closeListView(); });
  if (openSavedViewBtn) openSavedViewBtn.addEventListener("click", (e) => { e.preventDefault(); openSavedView(); });
}
  ...
  gpsButton.addEventListener("click", () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (state.map.flyTo) {
          state.map.flyTo([lat, lng], 13, { duration: 0.75 });
        } else {
