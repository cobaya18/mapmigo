...
sorted.forEach(({ place, index }) => {
  ...
  if (isOpen) {
    if (isSavedMode) {
      // Switch from Saved -> List without closing
      renderListView(state.currentVisible, "all");
      return;
    } else {
      // Already in List mode -> close
  ...
  if (isOpen) {
    if (!isSavedMode) {
      // Switch from List -> Saved without closing
      renderListView(saved, "saved");
      return;
    } else {
      // Already in Saved mode -> close
      overlay.classList.remove("open");
      return;
    }
  }

  // Closed -> open saved list
  renderListView(saved, "saved");
  overlay.classList.add("open");
}
...
// Drag-to-close on mobile for the list view sheet
const listHandle = overlay.querySelector(".list-sheet-handle");
if (listHandle) {
  let dragStartY = null;
  let currentY = null;
  let dragging = false;
  ...
  listHandle.addEventListener("touchstart", listTouchStart, { passive: true });
  listHandle.addEventListener("touchmove", listTouchMove, { passive: true });
  listHandle.addEventListener("touchend", listTouchEnd);
  listHandle.addEventListener("touchcancel", listTouchEnd);
}
